...
- **Partial-ing the plugin class across files works.** Part files in the plugin's folder merge
  into the entry class and are never tree-shaken (a part holding only hook methods still ships).
  Partial shared types merge the same way into one nested declaration. Staging deploys wrap each
  part's content in a `#region` naming its source file; release bundles carry no markers.

...
/// Carbon's protected-command table automatically. No corner close button on purpose: Cancel IS
/// the dismissal, and it runs a command so the server hears about it.
/// </summary>
public readonly struct ConfirmDialog
{
    ...
    private static Shell GetShell(Menu.Layer layer, string menuId)
    {
        ...
        shell.Payload = MenuShell.Build(menuId, menu =>
        {
            ...
            var window = root.AddPanel(MenuPosition.Center, new MenuOffset(-210f, -90f, 210f, 90f), MenuTheme.Background, needsCursor: true);

            // Message fills everything above the button row.
            window.AddContainer(new MenuPosition(0f, 0.35f, 1f, 1f), new MenuOffset(12f, 4f, -12f, -8f), shell.Message);

            // Button slots: pure containers so Set() controls the visuals per open.
            window.AddContainer(new MenuPosition(0f, 0f, 0.5f, 0.35f), new MenuOffset(12f, 12f, -6f, -8f), shell.Confirm);
            window.AddContainer(new MenuPosition(0.5f, 0f, 1f, 0.35f), new MenuOffset(6f, 12f, -12f, -8f), shell.Cancel);
        });
